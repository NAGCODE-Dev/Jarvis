...
if [ -x "$VENV_PYTHON" ]; then
  ...
  fail "virtualenv missing; run scripts/install_host.sh"
fi

if [ -f "$ROOT_DIR/.env" ]; then
  ok ".env found"
else
  fail ".env missing; run scripts/apply_quality_profile.sh or scripts/install_host.sh"
fi
...
if [ "$RAG_SMOKE" -eq 1 ]; then
  ...
  SAMPLE_DOC="$ROOT_DIR/data/knowledge/linux/verify-local.md"
  cat > "$SAMPLE_DOC" <<'EOF'
# Verify Local

Jarvis local verification document for Linux knowledge search.
EOF
  if curl -fsS -X POST "http://127.0.0.1:8000/api/knowledge/index" \
    -H 'content-type: application/json' \
    -d '{"domains":["linux"],"force":true}' \
    >/tmp/jarvis-rag-index.json 2>/tmp/jarvis-rag-index.err; then
  ...
  if curl -fsS -X POST "http://127.0.0.1:8000/api/knowledge/search" \
    -H 'content-type: application/json' \
    -d '{"query":"Jarvis local Linux verification","domain":"linux","top_k":1}' \
    >/tmp/jarvis-rag-search.json 2>/tmp/jarvis-rag-search.err; then
